...
const addCPU = () => {
    let empty = ITEMS.filter(item => item.textContent == '')
    let random = Math.floor(Math.random() * empty.length)
    if (empty.length > 0 && TURN.textContent == CIRCLE) {
        empty[random].textContent = CIRCLE;
        TURN.textContent = CROSS;
    }
    win()
}
...
const addSymbol = (e) => {
    if (e.target.textContent == '' && TURN.textContent == CROSS) {
        e.target.textContent = CROSS;
        TURN.textContent = CIRCLE;
        setTimeout(addCPU, 1000)
    }
}
...
const refresh = () => {
    ...
    unblockEvent()
    BODY.removeChild(document.querySelector('.score-board'))
    TURN.textContent = CROSS;
}
